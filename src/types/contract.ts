...
export interface SpiralHints {
  ...
  // Present only for uncorrelated (perp / directional) strategies. Frames how to read the APYs: every
  // leverageApyPct here is the annualized FINANCING CARRY (collateral yield − borrow cost, × leverage)
  // and is typically negative — it excludes the collateral's price change, which dominates the perp's
  // P&L. The position liquidates when the collateral falls to ltvPct.liquidation. Overridable opinion.
  profile?: {
    value: string; // "leveraged_perp"
